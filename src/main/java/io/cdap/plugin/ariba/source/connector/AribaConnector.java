...
public class AribaConnector implements Connector {
  ...
  AribaConnector(AribaConnectorConfig config) {
    this.config = config;
  }
  ...
  public void test(ConnectorContext connectorContext) throws ValidationException {
    ...
    } catch (AribaException | IOException e) {
  ...
  public ConnectorSpec generateSpec(ConnectorContext connectorContext, ConnectorSpecRequest connectorSpecRequest)
    ...
    try {
      ...
      LOG.error("Error in generating schema", e);
    }
    return specBuilder.addRelatedPlugin(new PluginSpec(AribaBatchSource.NAME, BatchSource.PLUGIN_TYPE,
                                                       properties)).build();
  }

  private JsonArray listTemplates(String pageToken, JsonArray jsonElements) {
    AribaServices aribaServices = new AribaServices(config);
    try {
      accessToken = aribaServices.getAccessToken();
    } catch (AribaException | IOException e) {
      LOG.error("unable to generate access token", e);
    }
    ...
    } catch (AribaException | IOException | InterruptedException e) {
      LOG.error("Unable to fetch response", e);
    }
